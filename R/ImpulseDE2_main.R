...
#' lsvecDispersionsExternal and vecSizeFactorsExternal, these are otherwise
...
#' @param lsvecDispersionsExternal: (vector length number of
...
runImpulseDE2 <- function(matCountData=NULL, 
  ...
  tm_runImpulseDE2 <- system.time({    
    ...
    # 2. Run DESeq2
    # Set parallelisation for DESeq2: without progress bar
    if(scaNProc > 1){ register(MulticoreParam(workers=scaNProc, progressbar=FALSE)) 
    ...
    if(is.null(lsvecDispersionsExternal)){
      ...
      lsvecDispersions <- lsvecDispersionsExternal
    }
    if(!is.null(dirTemp)){
      save(lsvecDispersions,file=file.path(dirTemp,"ImpulseDE2_lsvecDispersions.RData"))
    }
    ...
    # Set parallelisation for model fitting: with progress bar
    if(scaNProc > 1){ register(MulticoreParam(workers=scaNProc, progressbar=TRUE)) 
    } else { register(SerialParam()) }
    print("4. Fitting null and alternative model to the genes")
    tm_fitImpulse <- system.time({
      lsModelFits <- fitModels(
        matCountDataProc=matCountDataProc, 
        lsvecDispersions=lsvecDispersions,
