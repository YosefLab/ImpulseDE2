...
processData <- function(dfAnnotation, matCountData, boolCaseCtrl, vecConfounders, 
    ...
    # Checks whether elements are count data: non-negative integer finite
    # numeric elements.  Note that NA are allowed. Can be used to check
    # whether element is integer if NA is checked separately.
    ...
    checkData <- function(dfAnnotation, matCountData, boolCaseCtrl, vecConfounders, 
        vecDispersionsExternal, vecSizeFactorsExternal, strReportProcessing) {
        ...
        ### 2. Check annotation table content a) Check column names
        vecColNamesRequired <- c("Sample", "Condition", "Time", vecConfounders)
        if (!all(vecColNamesRequired %in% colnames(dfAnnotation))) {
            stop(paste0("Could not find column ", vecColNamesRequired[!(vecColNamesRequired %in% 
                colnames(dfAnnotation))], " in annotation table."))
        }
        ### b) Samples Check that sample name do not occur twice
        if (any(duplicated(dfAnnotation$Sample))) {
            stop(paste0("ERROR: [Annotation table] ", "Sample names must be unique: Sample(s) ", 
                paste0((dfAnnotation$Sample)[duplicated(dfAnnotation$Sample)], 
                  collapse = ","), " is/are duplicated."))
        }
        ### c) Time points
        ...
        if (!is.null(vecConfounders)) {
            # Check that model matrix based on confounding variables is full rank 1.
            # Dummy check: Check that number of batches for each confounding
            # variable is > 1
            for (confounder in vecConfounders) {
                if (length(unique(dfAnnotation[, confounder])) == 1) {
                  stop(paste0("ERROR: Model matrix based on confounding variables {", 
                    vecConfounders, "} is not full rank: Only one batch given for confounder ", 
        ...
        ### 3. Check expression table content Check that all entries in count data
        ### table occur in annotation table
        if (any(!(colnames(matCountData) %in% dfAnnotation$Sample))) {
            strReportProcessing <- paste0(strReportProcessing, "\n", "WARNING: The column(s) ", 
                paste0(as.character(colnames(matCountData)[!(colnames(matCountData) %in% 
                  dfAnnotation$Sample)]), collapse = ","), " in the count data table do(es) not occur in annotation table and will be ignored.")
        }
        ...
        ### Summarise which mode, conditions, samples and batch were found
        if (boolCaseCtrl) {
        ...
        for (tp in vecTimepoints) {
            strReportProcessing <- paste0(strReportProcessing, "\n", paste0("Case: Found the samples at time point ", 
                tp, ": ", paste0(dfAnnotation[(dfAnnotation$Time %in% tp) & 
                  (dfAnnotation$Condition %in% "case") & (dfAnnotation$Sample %in% 
                  colnames(matCountData)), ]$Sample, collapse = ","), collapse = ","))
        }
        if (boolCaseCtrl) {
            for (tp in vecTimepoints) {
                strReportProcessing <- paste0(strReportProcessing, "\n", 
                  paste0("Control: Found the following samples at time point ", 
                    tp, ":", paste0(dfAnnotation[dfAnnotation$Time %in% 
                      tp & dfAnnotation$Condition %in% "control" & dfAnnotation$Sample %in% 
            ...
            for (confounder in vecConfounders) {
                for (batch in unique(dfAnnotation[, confounder])) {
                  strReportProcessing <- paste0(strReportProcessing, "\n", 
                    ...
                    " and batch ", batch, ": ", paste0(dfAnnotation[dfAnnotation[, 
    ...
    # Add categorial time variable to annotation table which differentiates
    # case and control time points (given to DESeq2).  Add column with time
    # scalars with underscore prefix.  Add nested batch column if necssary
    # for running DESeq2.  Reduce to samples uesed.
    procAnnotation <- function(dfAnnotation, matCountData, boolCaseCtrl, 
        vecConfounders) {
        
        # Make sure all columns are not factors
        for (col in seq(1, dim(dfAnnotation)[2])) dfAnnotation[, col] <- as.vector(dfAnnotation[, 
            col])
        ...
        # Reduce to samples which occur in count table This allows to represent
        # entire library of samples in annotation file even if not all samples
        # were measured yet. Not measured samples which are not mentioned in
        # count table are ignored. Thereby, the same full annotation table can
        # be used throughout sample collection on incomplete data sets.
        ...
        if (boolCaseCtrl) {
            ...
            for (confounder in vecConfounders) {
                vecBatchesCase <- dfAnnotationProc[dfAnnotationProc$Condition == 
                  "case", confounder]
                vecNestedBatchesCase <- paste0("NestedBatch", match(vecBatchesCase, 
                  unique(vecBatchesCase)))
                vecBatchesCtrl <- dfAnnotationProc[dfAnnotationProc$Condition == 
                  "control", confounder]
                vecNestedBatchesCtrl <- paste0("NestedBatch", match(vecBatchesCtrl, 
                  unique(vecBatchesCtrl)))
    ...
    reduceCountData <- function(dfAnnotation, matCountDataProc) {
        ...
        strReportCountRed <- paste0("Input contained ", dim(matCountDataProc)[1], 
            " genes/regions.")
        ### 1. Columns (Conditions): Reduce expression table to columns of
        ...
        if (any(vecboolAllZeroSample)) {
            strReportCountRed <- paste0(strReportCountRed, "\n", "WARNING: Sample(s) ", 
                paste0(colnames(matCountDataProc)[vecboolAllZeroSample], 
                  collapse = ","), " only contain(s) zeros (and NAs). These samples are kept for analysis.")
        }
        
        ### 2. Rows (Genes): Exclude genes with only missing values (NAs)
        ...
        if (sum(!vecboolNonZeroGene) > 0) {
            strReportCountRed <- paste0(strReportCountRed, "\n", "WARNING: ", 
                sum(!vecboolNonZeroGene), " out of ", length(vecboolNonZeroGene), 
        ...
        strReportCountRed <- paste0(strReportCountRed, "\n", "Selected ", 
            dim(matCountDataProc)[1], " genes/regions for analysis.")
    ...
    strReportProcessing <- checkData(dfAnnotation = dfAnnotation, matCountData = matCountData, 
        boolCaseCtrl = boolCaseCtrl, vecConfounders = vecConfounders, vecDispersionsExternal = vecDispersionsExternal, 
        vecSizeFactorsExternal = vecSizeFactorsExternal)
    
    # Process annotation table
    dfAnnotationProc <- procAnnotation(dfAnnotation = dfAnnotation, matCountData = matCountData, 
        boolCaseCtrl = boolCaseCtrl, vecConfounders = vecConfounders)
    
    # Process raw counts
    matCountDataProc <- nameGenes(matCountDataProc = matCountData)
    lsReduceCounts <- reduceCountData(dfAnnotation = dfAnnotationProc, matCountDataProc = matCountDataProc)
    ...
    return(list(matCountDataProc = matCountDataProc, dfAnnotationProc = dfAnnotationProc, 
        vecSizeFactorsExternalProc = vecSizeFactorsExternalProc, vecDispersionsExternalProc = vecDispersionsExternalProc, 
