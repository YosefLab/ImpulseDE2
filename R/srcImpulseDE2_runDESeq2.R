...
runDESeq2 <- function(dfAnnotationProc, matCountDataProc, boolCaseCtrl, 
    vecConfounders) {
    
    # Get gene-wise dispersion estimates var = mean + alpha * mean^2, alpha
    # is dispersion DESeq2 dispersion is 1/size used dnbinom (used in cost
    # function for evaluation of likelihood)
    ...
    if (!boolCaseCtrl) {
        ...
        if (is.null(vecConfounders)) {
            ...
            # With batch correction Catch non full-rank design matrix e.g. batches
            # have of one confounder have mutually exclusive sets of time points
            tryCatch({
                dds <- suppressWarnings(DESeqDataSetFromMatrix(countData = matCountDataProc, 
                  colData = dfAnnotationProc, design = ~TimeCateg + Batch))
                dds <- estimateSizeFactors(dds)
                dds <- estimateDispersions(dds)
        ...
        # Case-control
        if (is.null(vecConfounders)) {
            # No batch correction Catch non full-rank design matrix e.g. conditions
            # have mutually exclusive sets of timepoints
            tryCatch({
                dds <- suppressWarnings(DESeqDataSetFromMatrix(countData = matCountDataProc, 
                  colData = dfAnnotationProc, design = ~Condition + Condition:TimeCateg))
                dds <- estimateSizeFactors(dds)
                dds <- estimateDispersions(dds)
            }, error = function(strErrorMsg) {
                print(strErrorMsg)
            ...
            # With batch correction Catch non full-rank design matrix e.g.  a)
            # batches have of one confounder have mutually exclusive sets of time
            # points or b) conditions have mutually exclusive sets of time points.
            # Note that full rank of design matrix of batches is checked in data
            # processing.  Note that design matrix of batches and condition is full
            # rank if batch matrix is full rank due to the nested batches trick
            # (using Condition:Batch below).  Therefore, error catching tailored to
            # a) and b).
            tryCatch({
                dds <- suppressWarnings(DESeqDataSetFromMatrix(countData = matCountDataProc, 
                  colData = dfAnnotationProc, design = ~Condition + Condition:TimeCateg + 
                ...
                print(strErrorMsg)
                print(paste0("WARNING: DESeq2 failed on full model - dispersions may be inaccurate.", 
                  "Estimating dispersions on reduced model.", " Supply externally generated dispersion parameters via ", 
                  "vecDispersionsExternal if there is a more accurate model for your data set."))
                warning("Warning generated in dispersion factor estimation, read stdout.")
            }, finally = {
                if (is.null(dds)) {
                  ...
                  if (!boolFullRankBatchTime) {
                    paste0("Model matrix based on confounding variables {", 
                      vecConfounders, "} and time is not full rank: There are confounding variables with ", 
                    ...
                    paste0("1. Model matrix based on confounding variables {", 
                      vecConfounders, "} and time is not full rank: There are confounding variables with ", 
                      " batch structures which are linear combinations of the time points.")
    ...
    # Catch dispersion trend outliers at the upper bounadry (alpha = 20
    # ->large variance) which contain zero measurements: The zeros throw off
    # the dispersion estimation in DESeq2 which may converge to the upper
    # bound even though the obesrved variance is small.  Avoid outlier
    # handling and replace estimates by MAP which is more stable in these
    # cases.  Note: the upper bound 20 is not exactly reached - there is
    # numeric uncertainty here - use > rather than ==
    vecindDESeq2HighOutliesFailure <- !is.na(mcols(dds)$dispOutlier) & mcols(dds)$dispOutlier == 
        TRUE & mcols(dds)$dispersion > (20 - 10^(-5)) & apply(matCountDataProc, 
        1, function(gene) any(gene == 0))
    ...
    if (sum(vecindDESeq2HighOutliesFailure) > 0) {
        print(paste0("Corrected ", sum(vecindDESeq2HighOutliesFailure), 
            " DESEq2 dispersion estimates which ", "to avoid variance overestimation and loss of discriminatory power for model selection."))
    }
