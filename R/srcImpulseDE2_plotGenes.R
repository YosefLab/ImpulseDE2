...
#' 
#' @export
plotGenes <- function(vecGeneIDs = NULL, scaNTopIDs = NULL, objectImpulseDE2, 
    ...
    # Check input
    if (is.null(objectImpulseDE2@lsModelFits)) 
        error("objectImpulseDE2 does not contain model fits. Run ImpulseDE2_main first.")
    ...
    if (is.null(objectImpulseDE2@lsModelFits$IdxGroups$case$lsvecBatchUnique) & 
    ...
    if (!is.null(vecRefPval)) 
        if (names(vecRefPval) != vecGeneIDs) 
    ...
    if (is.null(vecGeneIDs)) 
        vecGeneIDs <- objectImpulseDE2$dfImpulseDE2Results[with(objectImpulseDE2$dfImpulseDE2Results, 
            order(padj)), ]$Gene[1:scaNTopIDs]
    scaNIDs <- length(vecGeneIDs)
    lsgplotsID <- list()
    for (id in vecGeneIDs) {
        
        vecTimePointsFit <- seq(min(objectImpulseDE2@dfAnnotationProc$Time), 
            max(objectImpulseDE2@dfAnnotationProc$Time), length.out = 100)
        ...
        vecValueToPlotCase <- do.call(c, lapply(vecCaseBatchFactors, function(f) vecCaseImpulseValue * 
            f))
        
        if (boolCaseCtrl) {
            vecSamples <- objectImpulseDE2@dfAnnotationProc$Sample
            dfRaw <- data.frame(normCounts = objectImpulseDE2@matCountDataProc[id, 
                vecSamples]/objectImpulseDE2@vecSizeFactors[vecSamples], 
                ...
                Condition = objectImpulseDE2@dfAnnotationProc[vecSamples, 
                  ]$Condition)
            ...
            vecValueToPlotCtrl <- do.call(c, lapply(vecControlBatchFactors, 
                function(f) vecControlImpulseValue * f))
            vecValueToPlotComb <- do.call(c, lapply(vecCombinedBatchFactors, 
                function(f) vecCombinedImpulseValue * f))
            
            if (boolSimplePlot) {
                dfFit <- data.frame(time = rep(vecTimePointsFit, 3), value = c(vecCaseImpulseValue, 
                  vecControlImpulseValue, vecCombinedImpulseValue), Condition = c(rep("case", 
                  length(vecTimePointsFit)), rep("control", length(vecTimePointsFit)), 
                  rep("combined", length(vecTimePointsFit))))
                if (!is.null(vecBatchLabelsComb)) {
                  gplotGene <- ggplot() + geom_point(data = dfRaw, aes(x = time, 
                    y = normCounts, colour = Condition, shape = Batch)) + 
                    geom_line(data = dfFit, aes(x = time, y = value, colour = Condition))
                } else {
                  gplotGene <- ggplot() + geom_point(data = dfRaw, aes(x = time, 
                    y = normCounts, colour = Condition)) + geom_line(data = dfFit, 
                    aes(x = time, y = value, colour = Condition))
                }
            } else {
                dfFit <- data.frame(time = c(rep(vecTimePointsFit, length(vecCaseBatchFactors)), 
                  rep(vecTimePointsFit, length(vecControlBatchFactors)), 
                  rep(vecTimePointsFit, length(vecCombinedBatchFactors))), 
                  value = c(vecValueToPlotCase, vecValueToPlotCtrl, vecValueToPlotComb), 
                  Condition = c(rep(rep("case", length(vecTimePointsFit)), 
                    length(vecCaseBatchFactors)), rep(rep("control", length(vecTimePointsFit)), 
                    length(vecControlBatchFactors)), rep(rep("combined", 
                    length(vecTimePointsFit)), length(vecCombinedBatchFactors))), 
                  BatchFit = c(sapply(vecBatchLabelsCase, function(label) rep(label, 
                    length(vecTimePointsFit))), sapply(vecBatchLabelsCtrl, 
                    function(label) rep(label, length(vecTimePointsFit))), 
                    sapply(vecBatchLabelsComb, function(label) rep(label, 
            ...
            vecSamples <- objectImpulseDE2@dfAnnotationProc[objectImpulseDE2@dfAnnotationProc$Condition == 
                "case", ]$Sample
            if (!is.null(objectImpulseDE2@vecConfounders)) {
                vecBatches <- objectImpulseDE2@dfAnnotationProc[vecSamples, 
                  objectImpulseDE2@vecConfounders[1]]
            } else {
            ...
            dfRaw <- data.frame(normCounts = objectImpulseDE2@matCountDataProc[id, 
                vecSamples]/objectImpulseDE2@vecSizeFactors[vecSamples], 
                time = objectImpulseDE2@dfAnnotationProc[vecSamples, ]$Time, 
                Batch = vecBatches, condition = objectImpulseDE2@dfAnnotationProc[vecSamples, 
                  ]$Condition)
            
            if (boolSimplePlot) {
                dfFit <- data.frame(time = vecTimePointsFit, value = vecCaseImpulseValue, 
                  Batch = rep(vecBatchLabelsCase[1], length(vecTimePointsFit)))
                if (!is.null(vecBatchLabelsCase)) {
                  gplotGene <- ggplot() + geom_point(data = dfRaw, aes(x = time, 
                    y = normCounts, shape = Batch)) + geom_line(data = dfFit, 
                    aes(x = time, y = value, linetype = Batch))
                } else {
                  gplotGene <- ggplot() + geom_point(data = dfRaw, aes(x = time, 
                    y = normCounts)) + geom_line(data = dfFit, aes(x = time, 
                    y = value))
                }
            } else {
                dfFit <- data.frame(time = rep(vecTimePointsFit, length(vecCaseBatchFactors)), 
                  value = vecValueToPlotCase, condition = rep(rep("case", 
                    length(vecTimePointsFit)), length(vecCaseBatchFactors)), 
                  BatchFit = as.vector(sapply(vecBatchLabelsCase, function(label) rep(label, 
                    length(vecTimePointsFit)))))
                gplotGene <- ggplot() + geom_point(data = dfRaw, aes(x = time, 
                  y = normCounts, colour = Batch, shape = Batch)) + geom_line(data = dfFit, 
        ...
        if (!is.null(strNameRefMethod)) {
            gplotID <- gplotID + labs(title = paste0(id, ": ImpulseDE2 [", 
                round(log(objectImpulseDE2$dfImpulseDE2Results[id, ]$padj)/log(10), 
                  2), "] ", strNameRefMethod, " [", round(log(vecRefPval[id])/log(10), 
                  2), "]"))
        } else {
        ...
        if (boolMultiplePlotsPerPage) {
            ...
            for (p in seq(0, scaNPages)) {
                if (p < scaNIDs%/%scaNPlotsPerPage) {
                  vecidxPlots <- seq((p * scaNPlotsPerPage + 1), ((p + 1) * 
                    (scaNPlotsPerPage)))
                } else {
                  vecidxPlots <- seq((p * scaNPlotsPerPage + 1), scaNIDs)
                }
                print(plot_grid(plotlist = lsgplotsID[vecidxPlots], align = "h", 
                  nrow = scaNPlotsPerPage/2, ncol = 2, rel_widths = c(1, 
                    1), rel_heights = c(1, 1, 1)))
            }
        } else {
            pdf(dirFileOut)
