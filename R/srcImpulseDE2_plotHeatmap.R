...
#' 
#' @export
plotHeatmap <- function(objectImpulseDE2, strCondition, boolIdentifyTransients, 
    scaQThres = 0.01) {
    ...
    if (boolIdentifyTransients) {
        ...
        if (length(vecidxMonotonous) == 1) 
            matImpulseMonot <- t(matImpulseValue[vecidxMonotonous, ]) else matImpulseMonot <- matImpulseValue[vecidxMonotonous, ]
        ...
        vecidxMonotonousUpSort <- vecidxMonotonousUp[do.call(order, as.data.frame(matidxMinTimeSort[vecidxMonotonousUp, 
            1]))]
        vecidxMonotonousDown <- vecidxMonotonous[vecboolMonotonousDown]
        vecidxMonotonousDownSort <- vecidxMonotonousDown[do.call(order, 
            as.data.frame(matidxMinTimeSort[vecidxMonotonousDown, 1]))]
        
        # Fine sort transitive signals into off/on
        if (length(vecidxTransient) == 1) 
            matImpulseTransient <- t(matImpulseValue[vecidxTransient, ]) else matImpulseTransient <- matImpulseValue[vecidxTransient, ]
        ...
        vecidxTransientPeakSort <- vecidxTransientPeak[do.call(order, as.data.frame(matidxMaxTimeSort[vecidxTransientPeak, 
            1]))]
        ...
        vecidxTransientValleySort <- vecidxTransientValley[do.call(order, 
            as.data.frame(matidxMinTimeSort[vecidxTransientValley, 1]))]
        ...
        vecTrajectoryType <- c(rep("up", length(vecidxMonotonousUpSort)), 
            rep("down", length(vecidxMonotonousDownSort)), rep("*up", length(vecidxTransientPeakSort)), 
        ...
        lsvecGeneGroups <- list(transition_up = vecSignificantIDs[vecidxMonotonousUpSort], 
            ...
            transient_up = vecSignificantIDs[vecidxTransientPeakSort], transient_down = vecSignificantIDs[vecidxTransientValleySort])
    } else {
    ...
    matDataHeatZ <- do.call(rbind, lapply(seq(1, dim(matDataHeat)[1]), function(i) {
        (matDataHeat[i, ] - mean(matDataHeat[i, ], na.rm = TRUE))/sd(matDataHeat[i, 
            ], na.rm = TRUE)
    }))
    
    # Create heatmap of raw data, ordered by fits
    complexHeatmapRaw <- Heatmap(matDataHeatZ[vecidxAllSort, ], gap = unit(5, 
        "mm"), split = vecTrajectoryType, cluster_rows = FALSE, cluster_columns = FALSE, 
    ...
    # Create heatmap of raw data, ordered by fits
    complexHeatmapFit <- Heatmap(matDataHeatZ[vecidxAllSort, ], gap = unit(5, 
        "mm"), split = vecTrajectoryType, cluster_rows = FALSE, cluster_columns = FALSE, 
        heatmap_legend_param = list(title = "z-score"))
    
    return(list(complexHeatmapRaw = complexHeatmapRaw, complexHeatmapFit = complexHeatmapFit, 
