...
generate_background <- function(data_array, data_annotation, sigma_mat, 
  ...
  impulse_bg <- function(data_arr, data_annot, sigma_mat,  n_it = 100, 
    imp_fit, ctrl_tk = FALSE, ctrl_name = NULL,
    n_randoms_tot = 50000, no_proc = 4){
    ...
    if(ctrl_tk == TRUE){
      ...
      for(z in 1:dim(nRESD_1_s)[3]){
        # DEVELOPMENTAL NOTE: have to sample across replicates if want to deal with NaNs
        # Normalised residuals are drawn across time points for each gene x and replicate
        # z. The residuals are then scaled by the standard deviation of the timepoint
        # of the gene they have been sampled into.
        sampled_nRESD_1_s <- array(NA,c(dim(nRESD_1_s)[1],dim(nRESD_1_s)[2]))
        n_samples <- ncol(nRESD_1_s)
        for(x in 1:dim(nRESD_1_s)[1]){
          sampled_colInd <- sample(1:n_samples,n_samples,replace=TRUE)
          sampled_nRESD_1_s[x,] = nRESD_1_s[x,sampled_colInd,z]
        }
        scaled_sampled_nRESD_1_s <- sampled_nRESD_1_s * sigma_mat_rand
        # Generate bootstrapped samples:
        Y_star_s[,,z] = y_i_null_s_mat + scaled_sampled_nRESD_1_s
      }
      ...
      tm_fg <- system.time({
        ...
        impulse_fit_genes_bg <- impulse_fit(Y_star_s, data_annot, 
          sigma_mat_bg, 1, ctrl_tk, ctrl_name, fit_backg = TRUE )
      })
      ...
      if(reporting){
        print("sigma_mat_rand")
        print(sigma_mat_rand[1,])
        print("y_i_null_s")
        print(y_i_null_s[1])
        print("calc_impulse_comp_data_arr_rand")
        print(calc_impulse_comp_data_arr_rand[1,])
      }
      ...
      for(z in 1:dim(data_arr_rand)[3]){
        ...
        if(reporting){
          print("RESD_1_s")
          print((data_arr_rand[,,z] - calc_impulse_comp_data_arr_rand)[1,])
        }
      }
      if(reporting){
        print("data_arr_rand")
        print(data_arr_rand[1,,])
        print("nRESD_1_s")
        print(nRESD_1_s[1,,])
      }
      ...
      # Bootstrapped samples under H0: Mean + bootstrapped residual
      Y_star_s = array(NA,dim(nRESD_1_s))
      y_i_null_s_mat <- array(NA,c(dim(nRESD_1_s)[1],dim(nRESD_1_s)[2]))
      for(i_col in 1:ncol(y_i_null_s_mat)){
        y_i_null_s_mat[,i_col] <- y_i_null_s
      }
      for(z in 1:dim(nRESD_1_s)[3]){
        # DAVID resorted arrays a bit here
        # DEVELOPMENTAL NOTE: have to sample across replicates if want to deal with NaNs
        # Normalised residuals are drawn across time points for each gene x and replicate
        # z. The residuals are then scaled by the standard deviation of the timepoint
        # of the gene they have been sampled into.
        ...
        Y_star_s[,,z] = y_i_null_s_mat + scaled_sampled_nRESD_1_s
        if(reporting){
          print("sampled_nRESD_1_s")
          print(sampled_nRESD_1_s[1,])
          print("scaled_sampled_nRESD_1_s")
          print(scaled_sampled_nRESD_1_s[1,])
        }
      }
      colnames(Y_star_s) <- colnames(data_arr_rand)
      rownames(Y_star_s) <- paste("bg_",c(1:length(i_s)))
      ...
      # Compute standard deviations over bootstrapped replicates
      sigma_mat_bg <- compute_stdvs(observed_data=Y_star_s)
      # Compute weights for fitting
      weight_mat_bg <- compute_weights(observed_data=Y_star_s)
      if(reporting){
        print("sigma_mat_bg")
        print(sigma_mat_bg[1,])
        print("weight_mat_bg")
        print(weight_mat_bg[1,])
      }
      ...
      #      weight_mat_bg, n_it, control_timecourse = ctrl_tk, ctrl_name, fit_backg = TRUE)
      #})
      ...
      tm_fg <- system.time({
        #impulse_fit_genes_bg <- impulse_fit(Y_star_s, data_annot, weight_mat_bg, 
        #    n_it, ctrl_tk, ctrl_name, clustering_results_background, impulse_fit_clusters_bg, 
        #    fit_backg = TRUE )
        impulse_fit_genes_bg <- impulse_fit(Y_star_s, data_annot, weight_mat_bg, 
          n_it, ctrl_tk, ctrl_name, fit_backg = TRUE )
      })
    ...
    WSS0_s_b_rep <- array(0,c(dim(RESD_1_s_b)[1],dim(RESD_1_s_b)[3]))
    WSS1_s_b_rep <- array(0,c(dim(RESD_1_s_b)[1],dim(RESD_1_s_b)[3]))
    for (z in 1:dim(RESD_1_s_b)[3]){
      #F_background_rep[,z] = rowSums( ((RESD_0_s_b[,,z])^2 - (RESD_1_s_b[,,z])^2) *
      #    1/(sigma_mat_bg^2) )
      WSS0_s_b_rep[,z] <- rowSums( (RESD_0_s_b[,,z])^2 * 1/(weight_mat_bg^2) )
      WSS1_s_b_rep[,z] <- rowSums( (RESD_1_s_b[,,z])^2 * 1/(weight_mat_bg^2) )
    }
    WSS0_s_b <- rowSums(WSS0_s_b_rep)
    WSS1_s_b <- rowSums(WSS1_s_b_rep)
    #F_background = rowSums(F_background_rep)
    F_background <- (WSS0_s_b - WSS1_s_b)/WSS1_s_b
    
    if(reporting){
      # 9.  Plot sample background
      genesToPlot_bgID <- rownames(Y_star_s[1:min(36,dim(Y_star_s)[1]),,])
      genesToPlot_geneID <- rownames(data_arr[i_s,,])
      # Note: plots will be overwritten in by nodes in parallelised mode
      plot_impulse(genesToPlot_bgID,Y_star_s, data_annot, impulse_fit_genes_bg,
        ctrl_tk, ctrl_name, case_ind, file_name_part = "BG",
        title_line = "", sub_line = "")
      # Plot also originally selected points
      imp_fit_genesToPlot <- lapply(imp_fit,function(x){x <- (x[rownames(data_arr),])[genesToPlot_geneID,]})
      data_arr_rand_toPlot <- data_arr_rand
      rownames(data_arr_rand_toPlot) <- genesToPlot_geneID
      plot_impulse(genesToPlot_geneID,data_arr_rand_toPlot, data_annot, imp_fit_genesToPlot,
        ctrl_tk, ctrl_name, case_ind, file_name_part = "BGsample",
        title_line = "", sub_line = "")
    }
  ...
  if(n_rands>=50){
    ...
    assign("compute_weights", compute_stdvs, envir = my.env)
    assign("plot_impulse", plot_impulse, envir = my.env)
    ...
    #  "cluster_genes_for_impulse","impulse_fit","cost_fun_WLS_comp",
    #  "cost_fun_OLS_comp","compute_stdvs","compute_weights","plot_impulse"), envir = my.env)
    clusterExport(cl=cl, varlist=c("impulse_bg","n_rands","mc","data_array","data_annotation",
      "sigma_mat","n_iter","control_timecourse","control_name","imp_fit_genes","mc",
      "calc_impulse_comp","impulse_fit","cost_fun_WLS_comp","cost_fun_logl_comp",
      "cost_fun_OLS_comp","compute_stdvs","compute_weights","plot_impulse"), envir = my.env)
    ...
    res <- clusterEvalQ(cl,impulse_bg(data_arr=data_array, data_annot=data_annotation, 
      sigma_mat=sigma_mat, n_it=n_iter,imp_fit=imp_fit_genes, 
      ctrl_tk=control_timecourse, ctrl_name=control_name, 
      n_randoms_tot=n_rands, no_proc=mc))
    ...
    print("Unparallelized Background generation - low number of samples.")
    #res <- impulse_bg(data_arr=data_array, data_annot=data_annotation, 
    #    sigma_mat=sigma_mat, n_it=n_iter,imp_fit=imp_fit_genes, 
    #    ctrl_tk=control_timecourse, ctrl_name=control_name, n_clust=no_of_clus, 
    #    n_randoms_tot=n_rands, no_proc=1)
    res <- impulse_bg(data_arr=data_array, data_annot=data_annotation, 
      sigma_mat=sigma_mat, n_it=n_iter,imp_fit=imp_fit_genes, 
      ctrl_tk=control_timecourse, ctrl_name=control_name, 
      n_randoms_tot=n_rands, no_proc=1)
