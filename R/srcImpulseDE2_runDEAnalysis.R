...
runDEAnalysis <- function(objectImpulseDE2, boolCaseCtrl, vecConfounders, 
    ...
    if (!boolCaseCtrl) {
        ...
        vecConvergenceImpulse <- sapply(objectImpulseDE2@lsModelFits$case, 
            function(fit) fit$lsImpulseFit$scaConvergence)
        vecConvergenceConst <- sapply(objectImpulseDE2@lsModelFits$case, 
        ...
        if (!is.null(vecConfounders)) {
            scaNBatchFactorsFull <- sum(sapply(vecConfounders, function(confounder) {
                length(unique(objectImpulseDE2@dfAnnotationProc[objectImpulseDE2@dfAnnotationProc$Condition == 
                  "case", confounder])) - 1 + length(unique(objectImpulseDE2@dfAnnotationProc[objectImpulseDE2@dfAnnotationProc$Condition == 
        ...
        vecConvergenceImpulseCombined <- sapply(objectImpulseDE2@lsModelFits$combined, 
            function(fit) fit$lsImpulseFit$scaConvergence)
        vecConvergenceImpulseCase <- sapply(objectImpulseDE2@lsModelFits$case, 
            function(fit) fit$lsImpulseFit$scaConvergence)
        vecConvergenceImpulseControl <- sapply(objectImpulseDE2@lsModelFits$control, 
            function(fit) fit$lsImpulseFit$scaConvergence)
    }
    ...
    if (!boolCaseCtrl) {
        # Case-only
        dfDEAnalysis = data.frame(Gene = row.names(objectImpulseDE2@matCountDataProc), 
            p = vecPvalue, padj = vecPvalueBH, loglik_full = vecLogLikFull, 
            loglik_red = vecLogLikRed, df_full = rep(scaDegFreedomFull, 
                scaNGenes), df_red = rep(scaDegFreedomRed, scaNGenes), mean = vecMuCase, 
            converge_impulse = vecConvergenceImpulse, converge_const = vecConvergenceConst, 
            stringsAsFactors = FALSE)
    } else {
        # Case-control
        dfDEAnalysis = data.frame(Gene = row.names(objectImpulseDE2@matCountDataProc), 
            p = as.numeric(vecPvalue), padj = as.numeric(vecPvalueBH), loglik_full = vecLogLikFull, 
            loglik_red = vecLogLikRed, df_full = rep(scaDegFreedomFull, 
                scaNGenes), df_red = rep(scaDegFreedomRed, scaNGenes), mean = vecMuCombined, 
            converge_combined = vecConvergenceImpulseCombined, converge_case = vecConvergenceImpulseCase, 
            converge_control = vecConvergenceImpulseControl, stringsAsFactors = FALSE)
    }
    ...
    if (boolIdentifyTransients) {
        ...
        vecPvalueImpulseSigmoid <- pchisq(vecDevianceImpulseSigmoid, df = scaDegFreedomImpulse - 
        ...
        vecPvalueImpulseConst <- pchisq(vecDevianceImpulseConst, df = scaDegFreedomImpulse - 
            scaDegFreedomConst, lower.tail = FALSE)
        ...
        vecPvalueSigmoidConst <- pchisq(vecDevianceSigmoidConst, df = scaDegFreedomSigmoid - 
            scaDegFreedomConst, lower.tail = FALSE)
        vecPvalueSigmoidConstBH = p.adjust(vecPvalueSigmoidConst, method = "BH")
        
        # Add entries into DE table
        dfDEAnalysis$converge_sigmoid <- sapply(objectImpulseDE2@lsModelFits$case, 
        ...
        vecTimePointsCase <- sort(objectImpulseDE2@lsModelFits$IdxGroups$case$vecTimepointsUnique, 
            decreasing = FALSE)
        vecboolMonotonousImpulseTraject <- sapply(objectImpulseDE2@lsModelFits$case, 
            function(fit) {
                ...
                boolMonotonous <- (max(vecImpulseValues[2:length((vecImpulseValues) - 
                  1)]) <= max(vecImpulseValues[c(1, length(vecImpulseValues))]) & 
        ...
        # constant.
        dfDEAnalysis$isMonotonous <- (vecPvalueImpulseSigmoidBH > scaQThresTransients | 
            vecboolMonotonousImpulseTraject) & vecPvalueSigmoidConstBH <= 
            scaQThresTransients
    }
...
updateDEAnalysis <- function(objectImpulseDE2, scaQThresTransients = 0.001) {
    ...
    objectImpulseDE2 <- runDEAnalysis(objectImpulseDE2 = objectImpulseDE2, 
        boolCaseCtrl = objectImpulseDE2@boolCaseCtrl, vecConfounders = objectImpulseDE2@vecConfounders, 
