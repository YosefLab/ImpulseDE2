...
#   data_annotation: (Table samples x 2 [time, condition]) 
...
#       x number of runs [{combined, case, control} or only {case}])
...
#           Centroids of final clustering.
#       n_pre_clus: (scalar) Number of clusters used in pre-clustering
#       n_fine_clusters: (list scalars) Number of clusters in fine-clustering
#           per pre-cluster.
  ...
  for (c_runs in 1:runs){
    ...
    if(c_runs == 1){                # combined or only case if no control
      ...
      dat_pre = dat_pre[which(apply(dat_pre,1,function(x){100*
          sd(x)/(mean(x))})>= 2.5),]
      dat = t(apply(dat_pre,1,function(y){unlist(lapply(as.list(unique(
        as.numeric(as.character(data_annotation[
          !(data_annotation$Condition %in% control_name),"Time"])))),
        function(x){mean(y[data_annotation[
          !(data_annotation$Condition %in% control_name),"Time"] == x])}))}))
    ...
    if(is.null(no_of_clusters)){
      ...
      file_part = "bg"
      # Scale number of observed clusters up to relative size of bg data
      n_clus <- max(round(no_of_clusters[[c_runs*2 - 1]]*nrow(data_table)/
          n_genes),1)
      
      kmeans_clus <- Kmeans(dat_ztrans,n_clus,200,method="correlation")
      ### DAVID Bug?: why sample cluster lables in the first place?
      ...
      # This would be intuitive sampling
      #n_fine_clust_rand <- sample( round(no_of_clusters[[c_runs*2]]*
      #    nrow(data_table)/n_genes) ,n_clus,replace = T)
    }
    print(paste("--- Number of correlation-based pre-clusters: ",
      n_clus,sep=""))
    ...
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
    #~~~~~~~~~   Step 2: clustering based on euclidian distance   ~~~~~~~~~~~~~~#
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
    
    
    n_fine_clusters <- NULL
    cluster_means <-  NULL
    # Restrict maximum number of fine clusters per pre-cluster
    max_n_fine_clus <- 10
    kmeans_clus <- NULL
