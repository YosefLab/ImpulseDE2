...
#' 
#' @export
simulateDataSetImpulseDE2 <- function(vecTimePointsA, vecTimePointsB, vecBatchesA, 
    vecBatchesB, scaNConst, scaNImp, scaNLin, scaNSig, scaNRand = 0, scaSeedInit = 1, 
    scaMumax = 1000, boolOneConstMu = FALSE, scaSDExpressionChange = 1, 
    scaSDRand = NULL, scaMuSizeEffect = 1, scaSDSizeEffect = 0.1, scaMuBatchEffect = NULL, 
    ...
    names(vecSamplesA) <- sapply(seq(1, length(vecSamplesA)), function(i) {
        paste0("A_", vecSamplesA[i], "_Rep", match(i, which(vecSamplesA == 
            vecSamplesA[i])))
    })
    vecSamplesB <- vecTimePointsB
    if (!is.null(vecSamplesB)) {
        names(vecSamplesB) <- sapply(seq(1, length(vecSamplesB)), function(i) {
            paste0("B_", vecSamplesB[i], "_Rep", match(i, which(vecSamplesB == 
                vecSamplesB[i])))
        })
        boolCaseCtrl <- TRUE
    } else {
    ...
    dfAnnotation <- data.frame(Sample = names(vecSamples), Condition = rep("case", 
        length(vecSamples)), Time = vecSamples, Batch = c(vecBatchesA, vecBatchesB), 
        stringsAsFactors = FALSE)
    rownames(dfAnnotation) <- dfAnnotation$Sample
    if (boolCaseCtrl) {
        dfAnnotation[dfAnnotation$Sample %in% names(vecSamplesB), ]$Condition <- rep("control", 
            sum(dfAnnotation$Sample %in% names(vecSamplesB)))
    }
    ...
    if (scaNImp > 0) {
        vecImpulseIDs <- paste0(rep("gene", scaNImp), c((scaNConst + 1):(scaNConst + 
            scaNImp)))
    } else {
        vecImpulseIDs <- NULL
    }
    if (scaNLin > 0) {
        vecLinIDs <- paste0(rep("gene", scaNLin), c((scaNConst + scaNImp + 
            1):(scaNConst + scaNImp + scaNLin)))
    } else {
        vecLinIDs <- NULL
    }
    if (scaNSig > 0) {
        vecSigIDs <- paste0(rep("gene", scaNSig), c((scaNConst + scaNImp + 
            scaNLin + 1):(scaNConst + scaNImp + scaNLin + scaNSig)))
    } else {
        vecSigIDs <- NULL
    }
    if (scaNRand > 0) {
        vecRandIDs <- paste0(rep("gene", scaNRand), c((scaNConst + scaNImp + 
            scaNLin + scaNSig + 1):(scaNConst + scaNImp + scaNLin + scaNSig + 
            scaNRand)))
    } else {
        vecRandIDs <- NULL
    }
    ...
    if (scaNImp > 0) {
        ...
        lsMuImpulseHidden <- lapply(seq(1, scaNImp), function(gene) {
            evalImpulse(t = vecTimePointsUnique, beta = beta[gene], t1 = t1[gene], 
                t2 = t2[gene], h0 = h0[gene], h1 = h1[gene], h2 = h2[gene])[vecindTimePointAssign]
        })
        ...
        if (boolCaseCtrl & scaNImp > 1) {
            ...
            lsMuImpulseHiddenDE <- lapply(seq(1, scaNImpDE), function(gene) {
                evalImpulse(t = vecTimePointsUniqueB, beta = betaDE[gene], 
                  t1 = t1DE[gene], t2 = t2DE[gene], h0 = h0[gene], h1 = h1DE[gene], 
    ...
    if (scaNLin > 0) {
        ...
        set.seed(scaSeedInit + scaSeedsUsed)
        scaSeedsUsed <- scaSeedsUsed + 1
        vecFinalLevel <- vecInitialLevel * abs(rnorm(n = scaNLin, mean = 1, 
            sd = scaSDExpressionChange))
        ...
        if (boolCaseCtrl & scaNLin > 1) {
            ...
            set.seed(scaSeedInit + scaSeedsUsed)
            scaSeedsUsed <- scaSeedsUsed + 1
            vecFinalLevelDE <- vecInitialLevel[1:scaNLinDE] * abs(rnorm(n = scaNLinDE, 
                mean = 1, sd = scaSDExpressionChange))
            vecFinalLevelDE[vecFinalLevelDE < scaEps] <- scaEps
            # Evaluate linear functions
            scaDeltaTtot <- max(vecTimePointsUniqueB) - min(vecTimePointsUniqueB)
            matMuLinHiddenDE <- do.call(rbind, lapply(seq(1, scaNLinDE), 
                function(i) {
    ...
    if (scaNSig > 0) {
        ...
        if (boolCaseCtrl & scaNSig > 1) {
            ...
            # Evaluate sigmoid functions
            matMuSigHiddenDE <- do.call(rbind, lapply(seq(1, scaNSigDE), 
                function(i) {
                  evalSigmoid(t = vecTimePointsUniqueB, beta = vecBetaDE[i], 
                    t1 = vecT1DE[i], h0 = vech0[i], h1 = vech1DE[i])[vecindTimePointAssignB]
                }))
    ...
    matMuHiddenScaled <- matMuHidden * matrix(vecSizeFactorsHidden, nrow = dim(matMuHidden)[1], 
    ...
    if (boolCaseCtrl & length(vecBatchesUnique) == 2) {
        if (setequal(dfAnnotation[dfAnnotation$Batch == vecBatchesUnique[1], 
            ]$Sample, dfAnnotation[dfAnnotation$Condition == "case", ]$Sample) | 
            setequal(dfAnnotation[dfAnnotation$Batch == vecBatchesUnique[1], 
                ]$Sample, dfAnnotation[dfAnnotation$Condition == "ctrl", 
                ]$Sample)) {
            stop("Batch structure coincides with case-control structure.")
        }
    }
    if (length(vecBatchesUnique) > 1) {
        matBatchFactorsUnqiueHidden <- do.call(rbind, lapply(seq(1, scaNGenes), 
            function(i) {
                set.seed(scaSeedInit + scaSeedsUsed)
                scaSeedsUsed <- scaSeedsUsed + 1
                return(c(1, rnorm(n = length(vecBatchesUnique) - 1, mean = scaMuBatchEffect, 
                  sd = scaSDBatchEffect)))
            }))
    ...
    matObservedData <- do.call(rbind, lapply(seq(1, dim(matMuHiddenScaled)[1]), 
        function(gene) {
    ...
    if (!is.null(dirOutSimulation)) {
        ...
        if (length(vecBatchesUnique) > 1) {
            save(matBatchFactorsHidden, file = file.path(dirOutSimulation, 
                "Simulation_matBatchFactorsHidden.RData"))
        }
        if (boolCaseCtrl) {
            save(vecCaseCtrlDEIDs, file = file.path(dirOutSimulation, "Simulation_vecCaseCtrlDEIDs.RData"))
        }
    ...
    return(list(dfAnnotation = dfAnnotation, matObservedCounts = matObservedCounts))
}
